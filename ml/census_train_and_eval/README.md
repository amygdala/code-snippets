...
**In progress; not yet ready for review.**
...
This post ...  [** glosses over some of the details ..  assumes TF 1.4 or greater... **]
To see the details, visit this [Jupyter](xxx) [notebook](xxx).
(The example in the notebook is a slightly modified version of [this example](https://github.com/GoogleCloudPlatform/cloudml-samples/tree/master/census/estimator/trainer)).
...
See the accompanying [notebook](xxx) for the details of defining our Estimator, including specifying the expected format of the input data.
...
In the [notebook](xxx), we define a `build_estimator` function, which takes as input config info, and returns a `tf.estimator.DNNLinearCombinedClassifier` object.
...
We'll use [Datasets](https://www.tensorflow.org/api_docs/python/tf/contrib/data/Dataset) in `input_fn` to access our data. 
This API is part of TensorFlow 1.4, and is a new way to create input pipelines to TensorFlow models. The `Dataset` API is much more performant than using `feed_dict` or the queue-based pipelines, and it's [cleaner and easier](https://developers.googleblog.com/2017/09/introducing-tensorflow-datasets.html) to use.
...
The `input_fn` definition is the following. It uses a couple of helper functions defined in the [notebook](xxx).
`parse_label_column` is used to convert the label strings (in our case, ' <=50K' and ' >50K') into one-hot encodings.
...
Then, we'll use `input_fn` to define `train_input` and `eval_input` functions.  As we'll see below, these will be used to define a `TrainSpec` and `EvalSpec` used by `train_and_evaluate`.
...
You've just trained your model and exported the result in a format that makes it easy to use it for serving!


```sh
# List the directory that contains the model.  You'll use this info in the next section.
...
Once you've installed `gcloud`, we'll use it to easily make predictions using our local learned model, using  
...
Edit the following path to point to your `$output_dir/export/census/<timestamp>` subdirectory from the listing above, the one that contains `saved_model.pb`.
...
To interpret the prediction results: Class 0 means income <= 50k and Class 1 means income >50k.  
...
```python
# TODO -- add example that uses a GPU cluster to show how easy it is to swap back and forth?
```
...
```python
job_name = "census_job_%s" % (int(time.time()))
gcs_job_dir = "gs://aju-vtests2-ml-amy/amyu_census_14/%s" % job_name
%env GCS_TRAIN_FILE=gs://cloudml-public/census/data/adult.data.csv
%env GCS_EVAL_FILE=gs://cloudml-public/census/data/adult.test.csv
%env SCALE_TIER=BASIC_GPU
%env JOB_NAME=$job_name
%env GCS_JOB_DIR=$gcs_job_dir
```
